...
function skincare_product_store_admin_notice() {
    ...
    $skincare_product_store_dismissed = get_user_meta(get_current_user_id(), 'skincare_product_store_dismissed_notice', true);
    $skincare_product_store_current_page = '';
    if(isset($_GET['page'])) {
    	$skincare_product_store_current_page = admin_url( "admin.php?page=".sanitize_text_field($_GET["page"]));
    }

    // Display the notice only if not dismissed
    if (!$skincare_product_store_dismissed && $skincare_product_store_current_page != admin_url( "admin.php?page=wordclever-templates"))  {
        ?>
        		...
        		<div class="notice-content">
	        		<div class="notice-holder">
	                        ...
	                        <a href="javascript:void(0);" id="install-activate-button" class="button admin-button info-button">
							   <?php echo __('Getting started', 'skincare-product-store'); ?>
							</a>

							<script type="text/javascript">
	                            document.getElementById('install-activate-button').addEventListener('click', function () {
	                                const skincare_product_store_button = this;
	                                const skincare_product_store_redirectUrl = '<?php echo esc_url(admin_url("themes.php?page=skincare-product-store")); ?>';
